...
; set all textures to use when terrain will be constructed
...
; Build terrains
Define Imported = DefineTerrainTile(0, 0, 0, "terrain513.png", 0, 0)
BuildTerrain(0)
...
Procedure InitBlendMaps()
  Protected minHeight1.f = 70
  Protected fadeDist1.f = 40
  Protected minHeight2.f = 70
  Protected fadeDist2.f = 15
  Protected ty = 0
  Protected tx = 0
  Protected Size = TerrainTileLayerMapSize(0, tx, ty)
  Protected x, y
  Protected val.f, Height.f
  For y = 0 To Size-1
    For x = 0 To Size-1
...
Procedure BuildVehicle(*Vehicle.s_Vehicle)
  ...
  With *Vehicle
    ...
    Protected connectionHeight.f = 0.7

    Protected ChassisMesh = LoadMesh(#PB_Any, "chassis.mesh")
    Protected ChassisEntity = CreateEntity(#PB_Any, MeshID(ChassisMesh), #PB_Material_None, 0, 1.0, 0)
    ...
    Protected Wheel = LoadMesh(#PB_Any, "wheel.mesh")
    For i = 0 To 3
      \Wheels[i] = CreateEntity(#PB_Any, MeshID(Wheel), #PB_Material_None, 0, 0, 0, 32)
    Next

    Protected isFrontWheel = #True
