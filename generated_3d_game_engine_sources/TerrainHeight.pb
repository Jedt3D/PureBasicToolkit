...
Define Selected
Define.i light, dx, dy, tx, ty, Imported, Modify
...
; set all texture will be use when terrain will be constructed
...
Procedure InitBlendMaps()
  Protected.f minHeight1, fadeDist1, minHeight2, fadeDist2, Height, val
  Protected.i ty, tx, Size, y, x
  ...
  For ty = #TerrainMiniY To #TerrainMaxiY
    For tx = #TerrainMiniX To #TerrainMaxiX
      ...
      For y = 0 To Size-1
        For x = 0 To Size-1
          Height = TerrainTileHeightAtPosition(0, tx, ty, 1, x, y)
...
Procedure DoTerrainModify(tx, ty, wx.f, wy.f, wz.f, mBrushSizeTerrainSpace.f, TimeElapsed.f)
  Protected.f terrainSize, Pointx, Pointy
  Protected.i startx, starty, endx, endy, y, x
  Protected.f tsXdist, tsYdist, weight, addedHeight, newheight

  terrainSize = TerrainTileSize(0, tx, ty) - 1
  Pointx = TerrainTilePointX(0, tx, ty, wx, wy, wz)
  Pointy = TerrainTilePointY(0, tx, ty, wx, wy, wz)
  ...
  For y = starty To endy
    For x = startx To endx
      ...
      tsXdist = (x / terrainSize) - Pointx
      tsYdist = (y / terrainSize) - Pointy

      weight = Min(1.0, Sqr(tsYdist * tsYdist + tsXdist * tsXdist) / (0.5 * mBrushSizeTerrainSpace))
      weight = 1.0 - (weight * weight)
      addedHeight = weight * 250.0 * TimeElapsed

      newheight = GetTerrainTileHeightAtPoint(0, tx, ty, x, y) + addedHeight
