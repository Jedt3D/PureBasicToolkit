...
ExamineDesktops()
Define dx = DesktopWidth(0) * 0.9
Define dy = DesktopHeight(0) * 0.9
...
; sky
Define tx_sky = LoadTexture(#PB_Any, "sky.png")
SkyDome(TextureID(tx_sky), $cc6600, $0088ff, 3, 400, -0.5, 0)

; ocean
Define tx_water = LoadTexture(#PB_Any, "waternormal.png")
Define tx_foam  = LoadTexture(#PB_Any, "foam.png")
CreateWater(TextureID(tx_water), TextureID(tx_foam), $cc888800, $886666, #end_distance, 1.5, 1.5, 0.0, 0.7)

; ground
Define tx_ground = LoadTexture(#PB_Any, "Dirt.jpg")
...
Procedure CameraUserControl(camera, speed.f = 0.2, smooth.f = 0.1, yfixed.f = 1e10)
  ...
  If yfixed <> 1e10
    MoveCamera(camera, CameraX(camera), yfixed, CameraZ(camera), #PB_Absolute)
  EndIf
EndProcedure
...
Repeat
  ...
  ; sun height
  r + (KeyboardPushed(#PB_Key_PageUp) - KeyboardPushed(#PB_Key_PageDown)) * 0.004
  If r < 0 : r = 0 : ElseIf r > 1 : r = 1 : EndIf
  rr = Pow(r, 0.25)
