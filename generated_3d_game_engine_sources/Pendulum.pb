...
;
; ------------------------------------------------------------
; Generated from reference: Pendulum wave physics simulation
...
Define dx, dy, i
Define sol, support, support2, support3, support4, support5
Define.f KeyX, KeyY, keyz, dist, MouseX, MouseY
Define Quit
...
Repeat
  ...
  keyz = (-Bool(KeyboardPushed(#PB_Key_Down)) + Bool(KeyboardPushed(#PB_Key_Up))) * 0.2 + MouseWheel()
  dist + (keyz - dist) * 0.1
  RotateCamera(0, MouseY, MouseX, 0, #PB_Relative)
  MoveCamera  (0, KeyX, 0, -dist)

  ; Redraw the line3D to figure the pendulums' strings
  For i = 1 To #NBPENDULUM
    position = -9.3 + i * 1.10
    CreateLine3D(100 + i, EntityX(support) + position, EntityY(support), EntityZ(support), $77FF00, EntityX(sph(i)), EntityY(sph(i)), EntityZ(sph(i)), $77FF00)
  Next i
